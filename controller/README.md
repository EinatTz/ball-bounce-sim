...
- cmake 3.14 or newer — [download here](https://cmake.org/download/) (to build the controller library)
...
```bash
# macOS / Linux
cmake -S controller -B build -DCMAKE_BUILD_TYPE=Release
cmake --build build
cp build/libpaddle_controller.so controller.so      # Linux
cp build/libpaddle_controller.dylib controller.dylib  # macOS

# Windows
cmake -S controller -B build -DCMAKE_BUILD_TYPE=Release
cmake --build build --config Release
copy build\Release\paddle_controller.dll controller.dll
```
